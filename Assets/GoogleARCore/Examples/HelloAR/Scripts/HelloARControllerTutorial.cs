...
namespace GoogleARCore.HelloAR
{
    ...
    /// </summary>
    public class HelloARControllerTutorial : MonoBehaviour
    {
        ...
        public GameplayManagerTutorial manager;
        ...
        public void Update()
        {
            ...
            /* if (currPhase == GameplayManagerTutorial.GamePhase.Player1Hide || currPhase == GameplayManagerTutorial.GamePhase.Player2Hide)
             {
            ...
            //GameplayManagerTutorial.GamePhase currPhase = manager.WhichPhase();
            //Might need to get rid of this if but we'll test it
            if (manager.getAmtGhosts() < StaticVariables.Ghosts && !manager.isDone())
            {
                if (Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out hit))
                {
                    ...
                    if (manager.getAmtGhosts() < StaticVariables.Ghosts)
                    {
                        ...
                        ghostObject.transform.parent = anchor.transform;
                        ghostObject.name = "Ghost" + manager.getAmtGhosts();
                        manager.addGhost(ghostObject);
                    }
                }
            }
            else if (manager.getAmtGhosts() >= StaticVariables.Ghosts && !manager.isDone())
            {
                if (Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out hit))
                {
